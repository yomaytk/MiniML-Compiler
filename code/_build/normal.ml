...
let rec norm_exp (e: Syntax.exp) (f: cexp -> exp) = match e with
    ...
    | S.LoopExp (id, e1, e2) -> norm_exp e1 (fun x -> LoopExp (id, x, norm_exp e2 f))

    | S.RecurExp e -> 
        norm_exp e (fun x -> (match x with
                                    ValExp vc -> RecurExp vc
                                |   _         -> let nid = fresh_id "hi" in LetExp (nid, x, RecurExp (Var nid))))

    | S.TupleExp (e1, e2) -> let nid1 = fresh_id "ii" in let nid2 = fresh_id "ji" in
        norm_exp e1 (fun x -> 
            (match x with
                    ValExp vc1 -> norm_exp e2 (fun y -> 
                                    (match y with
                                            ValExp vc2 -> f (TupleExp(vc1, vc2))
                                        |   _          -> LetExp (nid2, y, f (TupleExp (vc1, Var nid2)))))
                |   _          -> norm_exp e2 (fun y -> 
                                    (match y with
